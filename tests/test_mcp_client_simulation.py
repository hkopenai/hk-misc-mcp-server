...
with a live MCP server over HTTP.
"""
...
class TestMCPClientSimulation(unittest.TestCase):
    ...
    This class contains test cases for simulating client interactions with a live MCP server,
    ensuring proper communication and response handling over HTTP.
    """
    server_process = None
    ...
    def setUp(self):
        ...
        self.server_process = subprocess.Popen(
            [sys.executable, "-m", "hkopenai.hk_misc_mcp_server", "--sse"],
    ...
    async def _call_tool_and_assert(self, tool_name, params):
        async with streamablehttp_client(self.SERVER_URL) as (
            ...
            async with ClientSession(read_stream, write_stream) as session:
                ...
                if not json_text:
                    # If json_text is empty, assume an empty list or dict based on expected return type
                    # For this specific tool, it returns a list, so assume empty list
                    data = []
                else:
    ...
    def test_get_auction_data_tool(self):
        """
        Test the 'get_auction_data' tool functionality.
        
        This test case verifies that the 'get_auction_data' tool can be called successfully
        and returns data in the expected format for the specified date range and language.
        """
        logger.debug("Testing 'get_auction_data' tool...")
        asyncio.run(
